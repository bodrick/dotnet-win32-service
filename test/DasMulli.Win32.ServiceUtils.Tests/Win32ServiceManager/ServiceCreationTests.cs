...
public partial class ServiceCreationTests
{
    ...
    private ServiceHandle CreateInvalidServiceHandle()
    {
        ...
        A.CallTo(() => invalidServiceHandle.IsInvalid).Returns(true);
        return invalidServiceHandle;
    }

    private void GivenCreatingAServiceIsImpossible() => A.CallTo(
            () =>
                _nativeInterop.CreateService(A<ServiceControlManager>._, A<string>._, A<string>._, A<ServiceControlAccessRights>._,
    ...
    private ServiceHandle GivenServiceCreationIsPossible(ServiceStartType serviceStartType)
    {
        ...
        A.CallTo(
                () =>
                    _nativeInterop.CreateService(_serviceControlManager, TestServiceName, TestServiceDisplayName,
            ...
            .ReturnsLazily(call =>
            {
                var serviceName = (string)call.Arguments[1];
    ...
    private void GivenTheServiceCanBeStarted(ServiceHandle service) => A
        .CallTo(() => _nativeInterop.StartService(service, A<uint>._, A<string[]>._))
        .Returns(true);
